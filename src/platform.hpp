...
/*
 * NOTE(Isaac): Down and right have positive values here
 */
enum ControllerAxis
{
  LEFT_STICK_X,
...
enum KeyboardKeys
{
  ...
  KEY_INTERNATIONAL1 = 135, // used on Asian keyboards
...
extern bool     g_keys[NUM_KEYBOARD_KEYS];
extern bool     g_mouseButtons[NUM_MOUSE_BUTTONS];
extern Vec<2u>  g_mousePosition;  // NOTE(Isaac): Relative to top-left of window

struct Controller
{
  Controller();

  bool    buttons[static_cast<unsigned int>(ControllerButton::NUM_BUTTONS)];
  int16_t axes[static_cast<unsigned int>(ControllerAxis::NUM_AXES)];
};

/*
 * Handle to something that's already been printed.
 * Can be used to replace a previously printed line or whatever.
 */
struct TermHandle
{
  unsigned int x;
